...
namespace PiwebSystemsPOS
{
    public partial class frmOrder : MetroFramework.Forms.MetroForm
    {
        ...
        //SalesOrder No
        //
        public string LoadSalesOrderNo()
        {
            var serialNo = "";
            int _result = 0;
        ...
        // Sales Quote Line No
        //
        public string LoadSalesQuoteLineNo()
        {
            var serialNo = "";
            string rec;

            cmd = new SqlCommand("SELECT isnull(MAX([SalesQuoteLineID]),100000) AS 'SalesQuoteLineNo' FROM [dbo].[SAL_SalesQuoteLines]", sqlConn);
            sda = new SqlDataAdapter(cmd);
            dt = new DataTable();
            sda.Fill(dt);

            rec = dt.Rows[0]["SalesQuoteLineNo"].ToString();
            ...
            if (dataGridView1.Rows.Count > 0)
            {
                foreach (DataGridViewRow row in dataGridView1.Rows)
                {
                    if (getProducts.Rows[0]["ProductName"].ToString() == row.Cells[0].Value.ToString())
                    {
                        ...
                        _lineAmount = Convert.ToDecimal(row.Cells[1].Value) * Convert.ToDecimal(getProducts.Rows[0]["UnitPrice"].ToString());
                        ...
                        if (!string.IsNullOrEmpty(taxGroupCode))
                        {
                            ...
                            taxAmount = (Convert.ToDecimal(getProducts.Rows[0]["UnitPrice"].ToString()) * ItemTaxRate) / 100;
                        }
                        found = true;
                        //qty = );
                    }
                ...
                if (!found)
                {
                    _price = Convert.ToDecimal(getProducts.Rows[0]["UnitPrice"].ToString());

                    dataGridView1.Rows.Add(getProducts.Rows[0]["ProductName"].ToString(), "1", String.Format("{0:N}", _price));
        ...
        private void frmOrder_Load(object sender, EventArgs e)
        {
            ...
            this.Text = "Sales " + TransactionsHelper.formType;
            lblCustomerName.Text = TransactionsHelper.CustomerName;

            //MessageBox.Show("Document No.:" + TransactionsHelper.DocumentNo);
        }

        private void PanelProductsView_Paint(object sender, PaintEventArgs e)
        {
        ...
        {
            frmCustomerList_Sales openCustomerListSales = new frmCustomerList_Sales();
            openCustomerListSales.ShowDialog();
            ...
            lblCustomerName.Text = customerDetails.Rows[0]["Name"].ToString();
        ...
        {
            string _formType = TransactionsHelper.formType;
            ...
            else
            {
                ...
                switch (_formType)
                {
                    case "Quote": // Create Quote Lines
                        //
                        // Quote
                        //
                        #region Create Sales Quote Lines
                        DocumentNo = TransactionsHelper.DocumentNo;
                        int lineNo = Convert.ToInt32(LoadSalesQuoteLineNo());
                        ...
                        for (int i = 0; i < dataGridView1.Rows.Count; i++)
                        {
                            ...
                            decimal unitPrice = Convert.ToDecimal(getProductDetails.Rows[0]["UnitPrice"].ToString());
                            if (active == 'Y')
                                status = "Yes";
                            else
                                status = "No";
                            ...
                            piwebDataOps.CreateSalesQuoteLines(lineNo.ToString(), DocumentNo, productCode, description, unitPrice, quantity, unitOfMeasure, unitPrice, 0, 0, "COMPLETED", taxGroupCode, "", "", lineTax, 0, 0, ItemTaxRate, 0, 0, includesVAT, taxGroupCode, 0, 0, 0, "", createdBy);
                            //MessageBox.Show("Sales Quote Line No: " + lineNo + "\nProduct Code: " + productCode + "\nUnit Price: " + unitPrice + "\nTax Group Code: " + taxGroupCode + "\nTax Rate: " + ItemTaxRate + "\nLine Tax: " + lineTax + "\nDiscount Group Code: " + discountGroupCode + "\nDescription: " + description + "\nActive: " + status + "\nQuantity: " + quantity.ToString() + "\nLine Amount: " + (Convert.ToDecimal(lineTotal + lineTax)).ToString(), "Message", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        }
                        //
                        // Update Sales Quote Header
                        piwebDataOps.UpdateSalesQuote(TransactionsHelper.DocumentNo, "COMPLETED", createdByDeviceId, createdBy);

                        //
                        // Clear dataGridView
                        dataGridView1.Rows.Clear();
                        MessageBox.Show("Quote No. " + DocumentNo + " has been created successfully", "Message", MessageBoxButtons.OK, MessageBoxIcon.Information);

                        #endregion
                        break;

                    case "Order":
                        MessageBox.Show("Post + Invoice?");
                        string deviceID = System.Environment.MachineName;
                        for (int i = 0; i < dataGridView1.Rows.Count; i++)
                        {
                            DocumentNo = TransactionsHelper.DocumentNo;
                            ...
                            taxindex = 1;
                            unitPrice = Convert.ToDouble(dataGridView1.Rows[i].Cells["amount"].Value);
                            ...
                            //Get Product Code
                            DataTable getProductDetails = piwebDataOps.GetProducts(DocumentNo);
                            productCode = getProductDetails.Rows[0]["No."].ToString();
                            ...
                            if (!string.IsNullOrEmpty(taxGroupCode))
                            {
                                ...
                                _tax1ID = getTax.Rows[0]["Tax Group Code"].ToString();
